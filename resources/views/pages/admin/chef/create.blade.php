<x-admin-layout>@section('title'){{ __('Add Chef') }}@endsection
    <x-slot name="header">
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                <div class="max-w-xl items-center mx-auto">
                    <section>
                    <x-form-section action="{{ route('admin.chefs.store') }}" enctype="multipart/form-data">
                            <x-slot name="title">
                                {{ __('Chef') }}
                            </x-slot>

                            <x-slot name="description">
                                {{ __("Add new chef information") }}
                            </x-slot>
                            ...
                            <x-slot name="form">
                                ...
                                <div class="col-span-6 sm:col-span-6">
                                    ...
                                    <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name')" placeholder="name" />
                                    <x-input-error class="mt-2" :messages="$errors->get('name')" />
                                </div>
                                <div class="col-span-6 sm:col-span-3">
                                    <x-input-label for="task" :value="__('Task')" />
                                    <x-text-icon-input id="task" type="text" class="mt-1 block w-full" name="task" :value="old('task')" placeholder="task username" icon="chef-hat" />
                                    <x-input-error class="mt-2" :messages="$errors->get('task')" />
                                </div>
                                <div class="col-span-6 sm:col-span-3">
                                    <x-input-label for="facebook" :value="__('Facebook')" />
                                    <x-text-icon-input id="facebook" type="text" class="mt-1 block w-full" name="facebook" :value="old('facebook')" placeholder="facebook username" icon="brand-facebook" />
                                    <x-input-error class="mt-2" :messages="$errors->get('facebook')" />
                                </div>
                                <div class="col-span-6 sm:col-span-3">
                                    <x-input-label for="twitter" :value="__('Twitter')" />
                                    <x-text-icon-input id="twitter" type="text" class="mt-1 block w-full" name="twitter" :value="old('twitter')" placeholder="twitter" icon="brand-twitter" />
                                    <x-input-error class="mt-2" :messages="$errors->get('twitter')" />
                                </div>
                                <div class="col-span-6 sm:col-span-3">
                                    <x-input-label for="instagram" :value="__('Instagram')" />
                                    <x-text-icon-input id="instagram" type="text" class="mt-1 block w-full" name="instagram" :value="old('instagram')" placeholder="instagram" icon="brand-instagram" />
